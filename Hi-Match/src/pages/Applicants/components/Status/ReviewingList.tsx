...
const ReviewingList = ({ reviewing }: ReviewingListProps) => {
    ...
    const submitAllRejected = () => {
        mutate(
            { postingNo: Number(selectedPostNo), category: "PROGRESS" },
            {
                onSuccess: () => {
                    toast.success(
                        "서류 검토 목록의 지원자들이 불합격 처리되었습니다."
                    );
    ...
    return (
        <div className="reviewing border-gray03 scroll-custom h-175 min-h-175 space-y-5 overflow-y-auto rounded-[10px] border border-solid bg-white p-5">
