...
const CoverLetterFormEdit = ({
    ...
    // ✅ 최초 렌더링 시, question으로 type/district 역추적
    useEffect(() => {
        const matched = COVER_LETTER_QUESTINONS.find(q =>
            q.districts.some(d => d.value === form.question)
        );
        if (matched) {
            setSelectedType(matched.label);
            setSelectedDistrict(form.question);
        }
    }, []);
...
export default CoverLetterFormEdit;
