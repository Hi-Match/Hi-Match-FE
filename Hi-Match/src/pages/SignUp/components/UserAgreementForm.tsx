...
type SingleCheckKey = Exclude<keyof UserAgreementFormProps["checkList"], "all">;

const UserAgreementForm = ({
    checkList,
    setCheckList,
}: UserAgreementFormProps) => {
    // 약관 전체 동의 함수
    ...
    const handleCheckSingle = (check: SingleCheckKey) => {
        ...
        const { age, use, personal } = updated;
        updated.all = age && use && personal;
            ...
            <div className="space-y-3.75">
                <div className="relative flex w-full items-center justify-start">
                    <input
                        ...
                        onChange={() => handleCheckSingle("age")}
                    />
                    <label
                        htmlFor="age"
                ...
                <div className="relative flex w-full items-center justify-start">
                    <input
                        ...
                        id="personal"
                        className="peer hidden"
                        checked={checkList.personal}
                        onChange={() => handleCheckSingle("personal")}
                    />
                    <label
                        htmlFor="personal"
                        className="text-gray01 cursor-pointer pl-[26px] text-base font-medium before:absolute before:top-1 before:left-0 before:h-4 before:w-4 before:rounded-sm before:border before:border-gray-300 before:bg-white peer-checked:before:bg-blue-500"
